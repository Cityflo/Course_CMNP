...
class CorrelatedJitter_Trains:
    
    """
    Class for creating groups of spike trains, with
    exponential correlations within groups created by copying spikes
    and including jitter. 
    Poisson statistics are maintained at the single train level.
    Based on Romain Brette, 2008. 
    """
    
    def get_list_of_trains(self,c1,c2,frate,tau_c):
        ...
        for group in range(2):
            ...
            # generate a source spike train (Poisson)
            ...
            for syn in range(1,int(nin_tot/2)):
                ...
                for ii in range(0,len(spike_time_source)):
                    if nr.uniform(0,1)<=copy_probab:
                        
                        ### jitter spike correlations
                        sign_jitter = 1 - (nr.randint(0,2)*2)
                        
                        new_spike = spike_time_source[ii] + (nr.exponential(tau_c))*sign_jitter
                        spike_time_new.append(new_spike)
