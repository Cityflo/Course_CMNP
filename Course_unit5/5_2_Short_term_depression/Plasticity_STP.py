...
class STPlasticity:
    ...
    ''''' This class defines a STP object.
    Both short-term facilitation (STF) and short-term depression (STD)
    are included.
        ...
        for syn in range(len(pre_spikes_all)):
            ...
            for sp in range(len(pre_spikes_all[syn])):
                ...
                if pre_spikes_all[syn][sp]>=tt-time_step_sim and pre_spikes_all[syn][sp]<tt+time_step_sim:
                    ...
                    x_vec[syn]=x_vec[syn]+(1-x_vec[syn])/tau_d-(u_vec[syn]*x_vec[syn]) 
            ...
            if spike_or_not==0:
            # recovery of the STF and STD variable for this synapse
                # Short-term faciliation
                u_vec[syn]=u_vec[syn]-u_vec[syn]/tau_f
                
                # short-term depression
                x_vec[syn]=x_vec[syn]+(1-x_vec[syn])/tau_d
