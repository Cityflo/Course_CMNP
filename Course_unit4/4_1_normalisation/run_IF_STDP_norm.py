...
synaptic normalisation in the excitatory input synapses.
The inputs are excitatory and inhibitory Poisson
distributed inputs, with correlations between the excitatory inputs only.
Synaptic weight evolutions and distributions are shown.
...
W_tot         = Parameters_Int_and_Fire.W_tot
etha_SN       = Parameters_Int_and_Fire.etha_SN
...
while tt <= t_max:
    ...
    if tt%1000==0 and sum(w_e_vec_tt) != 0:    
        sfac = 1 + etha_SN*(W_tot/sum(w_e_vec_tt)-1)
        w_e_vec_tt = [i * sfac for i in w_e_vec_tt]
        
        # record spikes at the same time
...
#plt.tight_layout()
#plt.show()
#fig1.savefig('4_1_STDP_norm_wevol.png')

# weight distributions of the two excitatory groups
res = 0.01
bn = np.arange(0,0.3+res,res)

ax3 = fig1.add_subplot(gs[0,-3::])
ax3.hist(w_e_storage[-1,0:int(numb_exc_syn*0.5)],bins = bn,lw=2,histtype='step',color='m',label='Correl.: ' + str(c1))
ax3.hist(w_e_storage[-1,int(numb_exc_syn*0.5):numb_exc_syn],bins=bn,lw=2,histtype='step',color='g',label='Correl.: ' + str(c2))
ax3.set_xlabel('Synaptic Weight')
ax3.set_ylabel('Frequency')
ax3.legend()
plt.tight_layout()
plt.show()
fig1.savefig('4_1_STDP_norm_wevol.png')
#fig2.savefig('4_1_STDP_norm_wdist.png')